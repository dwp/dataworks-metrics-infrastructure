  ...
  vars = {
    ...
    environment_variables = jsonencode([
      ...
      },
      {
        ...
        "value" : "${join(" ", concat(formatlist("${var.name}-${var.secondary}.%s.services.${var.parent_domain_name}", "${local.master_peerings[local.slave_peerings[local.environment]]}"), formatlist("thanos-store.services.%s.${var.parent_domain_name}", "${local.master_peerings[local.management_envs]}")))}"
      },
      {
