...
resource "aws_security_group_rule" "allow_prometheus_egress_analytical_frontend_service" {
  ...
  type                     = "egress"
  protocol                 = "tcp"
  from_port                = data.terraform_remote_state.analytical-frontend-service.frontend_service.value.service_port
  to_port                  = data.terraform_remote_state.analytical-frontend-service.frontend_service.value.service_port
  security_group_id        = aws_security_group.prometheus.id
  source_security_group_id = data.terraform_remote_state.analytical-frontend-service.ecs_tasks.id
}
