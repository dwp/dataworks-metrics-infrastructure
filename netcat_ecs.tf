//resource "aws_ecs_task_definition" "netcat" {
...
//  container_definitions    = "[${data.template_file.netcat_definition[local.primary_role_index].rendered}]"
//  tags                     = merge(local.tags, { Name = var.name })
//}
//
//data "template_file" "netcat_definition" {
//  count    = local.is_management_env ? 1 : 0
...
//    image_url     = format("%s:%s", data.terraform_remote_state.management.outputs.ecr_thanos_url, var.image_versions.netcat)
...
//}
//
//resource "aws_ecs_service" "netcat" {
//  count                              = local.is_management_env ? 1 : 0
//  name                               = "netcat"
//  cluster                            = aws_ecs_cluster.metrics_ecs_cluster.id
//  task_definition                    = aws_ecs_task_definition.netcat[local.primary_role_index].arn
...
//
//  service_registries {
//    registry_arn   = aws_service_discovery_service.netcat[local.primary_role_index].arn
...
//}
//
//resource "aws_service_discovery_service" "netcat" {
//  count = local.is_management_env ? 1 : 0
//  name  = "netcat"
//
//  dns_config {
